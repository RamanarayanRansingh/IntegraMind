...
NUM_API_KEYS = 7
BATCHES_DIR = 'evaluation_batches'

def load_api_keys(num_keys: int) -> List[str]:
...
def split_test_data(test_cases: List[Dict], num_batches: int) -> List[List[Dict]]:
    """Divide test cases into equal batches"""
    batch_size = len(test_cases) // num_batches
    remainder = len(test_cases) % num_batches
    ...
    for i in range(num_batches):
        ...
        batches.append(test_cases[start_idx:start_idx + size])
        start_idx += size
    
    return batches

def create_batch_evaluator_script(batch_num: int, num_batches: int) -> str:
    """Create evaluator script for a batch"""
...
INPUT_DATA_FILE = os.path.join(SCRIPT_DIR, 'batch_{batch_num}_testdata.json')
OUTPUT_RESULTS_FILE = os.path.join(SCRIPT_DIR, 'batch_{batch_num}_results.json')
DEBUG_OUTPUT_FILE = os.path.join(SCRIPT_DIR, 'batch_{batch_num}_debug.json')
RATE_LIMIT_DELAY = 7
BATCH_NUM = {batch_num}

API_KEY = os.getenv("GOOGLE_API_KEY_{batch_num}")
if not API_KEY:
    print(f"ERROR: GOOGLE_API_KEY_{batch_num} not found")
    sys.exit(1)
...
def main():
    """Main splitter logic"""
    print("\nPARALLEL BATCH EVALUATOR - SETUP\n")
    
    api_keys = load_api_keys(NUM_API_KEYS)
    ...
    batches = split_test_data(test_cases, len(api_keys))
    Path(BATCHES_DIR).mkdir(exist_ok=True)
    
    for batch_num, batch_cases in enumerate(batches, 1):
        batch_data_file = os.path.join(BATCHES_DIR, f"batch_{batch_num}_testdata.json")
        batch_data = {
            "batch_number": batch_num,
        ...
        batch_script_file = os.path.join(BATCHES_DIR, f"batch_{batch_num}_evaluator.py")
        script_content = create_batch_evaluator_script(batch_num, len(api_keys))
        
        with open(batch_script_file, 'w', encoding='utf-8') as f:
            f.write(script_content)
        
        print(f"Batch {batch_num}: {len(batch_cases)} cases, uses GOOGLE_API_KEY_{batch_num}")
    
    print(f"\nRun all batches from {BATCHES_DIR}:")
    print("cd evaluation_batches")
    for i in range(1, len(api_keys) + 1):
        print(f"python batch_{i}_evaluator.py &")
